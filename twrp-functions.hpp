...
public:
	...
	static std::string to_string(unsigned long long value); //convert ull to string
	...
	static void copy_pstore_log(string curr_storage); // Copy Logcat to Current Storage
	...
	static bool Get_Service_From_FileName(std::string Path, std::string Service, std::string &Ret);
