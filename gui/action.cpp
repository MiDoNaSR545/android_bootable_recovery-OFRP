...
int GUIAction::copylog(std::string arg __unused)
{
  ...
  if (!simulate)
	...
	TWFunc::copy_logcat(curr_storage);
      ...
      if (copy_pstore_log)
	TWFunc::copy_pstore_log(curr_storage);
...
void GUIAction::find_magisk(){ //[f/d]
  ...
  for (int i = 0; i < zip_queue_index; i++)
    if (zip_queue[i] == std::string("/FFiles/OF_Magisk/") + FOX_MAGISK_ZIP_INSTALLER)
      found = 1;
